					...
					<div className="container mx-auto px-4 flex justify-center">
						<div className="grid grid-cols-1 md:grid-cols-2 gap-8 justify-center">
							...
							<Card className="border-2 border-accent shadow-2xl flex flex-col relative overflow-hidden">
								<div className="bg-accent text-accent-foreground text-center py-1 text-sm font-semibold">Best Value</div>
								<CardHeader className="text-center">
									<CardTitle className="text-3xl font-bold font-headline">Premium</CardTitle>
									<CardDescription>One time payment, lifetime access</CardDescription>
								</CardHeader>
								<CardContent className="flex-grow">
									<div className="text-5xl font-bold text-center mb-6">
										$10<span className="text-lg font-normal text-muted-foreground">/one time</span>
									</div>
									<ul className="space-y-3 text-foreground">
										{features.map((f) => (
											<li key={f.name} className="flex items-center gap-2">
												...
												<span>
													{f.name} {f.soon && <span className="text-xs text-accent">(Soon)</span>}
												</span>
											</li>
										))}
									</ul>
